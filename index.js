...
<div class="artist-container">
  ...
  `}function v(t){if(t==null||t==="")return!0;const e=String(t).trim().toLowerCase();return e===""||e==="information missing"}function R(t,e){const s=!v(t),n=!v(e);return!s&&!n?"Information missing":s&&!n?`${t}–present`:!s&&n?`until ${e}`:`${t}–${e}`}async function N(t){o.classList.remove("is-hidden"),document.body.style.overflow="hidden",J();try{Y();const e=await H(t),n=(await F(t)).albumsList||[];A.innerHTML=K(e,n)}catch(e){console.log(e),A.innerHTML=`
      <p class="error">Failed to load artist information</p>
    `}finally{j()}}function u(){Z(),o.classList.add("is-hidden"),document.body.style.overflow="",A.innerHTML=""}function E(t){t.key==="Escape"&&u()}function S(t){t.target===o&&u()}function J(){w.addEventListener("click",u),document.addEventListener("keydown",E),o.addEventListener("click",S)}function Z(){w.removeEventListener("click",u),document.removeEventListener("keydown",E),o.removeEventListener("click",S)}function Y(){L.classList.remove("is-hidden")}function j(){L.classList.add("is-hidden")}const X=document.getElementById("artistsList");X.addEventListener("click",async t=>{if(t.target.classList.contains("artist-more")){const e=t.target.closest(".artist-card");if(!e)return;const s=e.dataset.id;try{await N(s)}catch(n){console.log(n)}}});function _(t){if(!t)return"--:--";const e=Math.floor(t/1e3),s=Math.floor(e/60),n=e%60;return`${s}:${n.toString().padStart(2,"0")}`}function tt(t){const{strTrack:e,intDuration:s,movie:n}=t;return`
    <li class="track">
