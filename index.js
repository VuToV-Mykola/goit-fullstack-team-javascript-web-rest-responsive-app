import{a as d,S as T,N as Q,P as x,R as C}from"./assets/vendor-CoX6hYw-.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&n(i)}).observe(document,{childList:!0,subtree:!0});function s(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(a){if(a.ep)return;a.ep=!0;const r=s(a);fetch(a.href,r)}})();const u="https://sound-wave.b.goit.study/api",U=1,D=8,H=10;async function q(t=U,e=D){return(await d.get(`${u}/artists`,{params:{page:t,limit:e}})).data}async function F(t){if(!t)throw new Error("Artist id is required");return(await d.get(`${u}/artists/${t}`)).data}async function O(t){if(!t)throw new Error("Artist id is required");return(await d.get(`${u}/artists/${t}/albums`)).data}async function P(){try{return(await d.get(`${u}/feedbacks`)).data}catch(t){throw new Error("Failed to load feedbacks",{cause:t})}}document.querySelectorAll(".nav-list a").forEach(t=>{t.addEventListener("click",()=>{document.getElementById("nav-toggle").checked=!1})});function V(){const t=document.querySelector("#btn-hero-js");t&&t.addEventListener("click",()=>{const e=document.querySelector("#artists");e&&e.scrollIntoView({behavior:"smooth",block:"start"})})}document.addEventListener("DOMContentLoaded",()=>{V()});const z=document.getElementById("artistsList"),p=document.getElementById("loadMoreBtn"),y=document.getElementById("artistsLoader");let m=1;const g=8;async function k(){try{G();const t=await q(m,g),e=t.artists||t;R(e),e.length<g?p.classList.add("is-hidden"):p.classList.remove("is-hidden"),m+=1}catch(t){console.error("Failed to load artists:",t)}finally{W()}}function R(t){if(!Array.isArray(t))return;const e=t.map(({_id:s,strArtist:n,strArtistThumb:a,genres:r,strBiographyEN:i})=>{const o=r.map(l=>`<span class="artist-genre">${l}</span>`).join("");return`
      ...
      `}).join("");z.insertAdjacentHTML("beforeend",e)}function G(){y.classList.remove("is-hidden")}function W(){y.classList.add("is-hidden")}k();p.addEventListener("click",k);const A=document.querySelector(".modal-content"),w=document.querySelector(".modal-close-btn"),c=document.querySelector(".backdrop"),L=document.getElementById("modalLoader");function K(t,e=[]){const{strArtist:s,strArtistThumb:n,intFormedYear:a,intDiedYear:r,strGender:i,intMembers:o,strCountry:l,strBiographyEN:B,genres:M}=t;return`
...
<div class="artist-container">
  ...
  <div class="artist-info">
    ...
    <ul class="info-genres">
      ${(M||[]).map(I=>`<li class="info-genre">${I||""}</li>`).join("")}
    </ul>
   </div>
  </div>
</div>
  ...
  `}function b(t){if(t==null||t==="")return!0;const e=String(t).trim().toLowerCase();return e===""||e==="information missing"}function Y(t,e){const s=!b(t),n=!b(e);return!s&&!n?"Information missing":s&&!n?`${t}–present`:!s&&n?`until ${e}`:`${t}–${e}`}async function N(t){c.classList.remove("is-hidden"),document.body.style.overflow="hidden",J();try{j();const e=await F(t),a=[...(await O(t)).albumsList||[]].sort((r,i)=>{const o=r.intYearReleased??0;return(i.intYearReleased??0)-o});A.innerHTML=K(e,a)}catch(e){console.log(e),A.innerHTML=`
      <p class="error">Failed to load artist information</p>
    `}finally{X()}}function f(){Z(),c.classList.add("is-hidden"),document.body.style.overflow="",A.innerHTML=""}function E(t){t.key==="Escape"&&f()}function S(t){t.target===c&&f()}function J(){w.addEventListener("click",f),document.addEventListener("keydown",E),c.addEventListener("click",S)}function Z(){w.removeEventListener("click",f),document.removeEventListener("keydown",E),c.removeEventListener("click",S)}function j(){L.classList.remove("is-hidden")}function X(){L.classList.add("is-hidden")}const _=document.getElementById("artistsList");_.addEventListener("click",async t=>{if(t.target.classList.contains("artist-more")){const e=t.target.closest(".artist-card");if(!e)return;const s=e.dataset.id;try{await N(s)}catch(n){console.log(n)}}});function tt(t){if(!t)return"--:--";const e=Math.floor(t/1e3),s=Math.floor(e/60),n=e%60;return`${s}:${n.toString().padStart(2,"0")}`}function $(t){const e=t.movie||t.strMusicVid||t.strVideo||null;if(!e||typeof e!="string")return null;const s=e.trim();return s||null}function et(t,e=!0){const{strTrack:s,intDuration:n}=t,a=$(t);return`
    <li class="track${e?"":" track--no-link"}">
      <span class="track-name">${s}</span>
      <span class="track-time">${tt(n)}</span>
      ...
      ${e&&a?`
        <a
          ...
          aria-label="Open ${s||"track"} on YouTube">
  ...
  `}function st(t){return(t||[]).some(e=>$(e))}function nt(t){const{strAlbum:e,intYearReleased:s,tracks:n=[]}=t,a=st(n);return`
    <div class="album-card">
      ...
      <div class="tracks-header${a?"":" tracks-header--no-link"}">
        <span>Track</span>
        <span>Time</span>
        ${a?"<span>Link</span>":""}
      </div>
