import{a as d,S as I,N as T,P as Q,R as x}from"./assets/vendor-CoX6hYw-.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&n(i)}).observe(document,{childList:!0,subtree:!0});function s(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(a){if(a.ep)return;a.ep=!0;const r=s(a);fetch(a.href,r)}})();const u="https://sound-wave.b.goit.study/api",C=1,U=8,D=10;async function q(t=C,e=U){return(await d.get(`${u}/artists`,{params:{page:t,limit:e}})).data}async function H(t){if(!t)throw new Error("Artist id is required");return(await d.get(`${u}/artists/${t}`)).data}async function P(t){if(!t)throw new Error("Artist id is required");return(await d.get(`${u}/artists/${t}/albums`)).data}async function F(){try{return(await d.get(`${u}/feedbacks`)).data}catch(t){throw new Error("Failed to load feedbacks",{cause:t})}}document.querySelectorAll(".nav-list a").forEach(t=>{t.addEventListener("click",()=>{document.getElementById("nav-toggle").checked=!1})});function O(){const t=document.querySelector("#btn-hero-js");t&&t.addEventListener("click",()=>{const e=document.querySelector("#artists");e&&e.scrollIntoView({behavior:"smooth",block:"start"})})}document.addEventListener("DOMContentLoaded",()=>{O()});const V=document.getElementById("artistsList"),A=document.getElementById("loadMoreBtn"),k=document.getElementById("artistsLoader");let b=1;const v=8;async function L(){try{R();const t=await q(b,v),e=t.artists||t;z(e),e.length<v?A.classList.add("is-hidden"):A.classList.remove("is-hidden"),b+=1}catch(t){console.error("Failed to load artists:",t)}finally{G()}}function z(t){if(!Array.isArray(t))return;const e=t.map(({_id:s,strArtist:n,strArtistThumb:a,genres:r,strBiographyEN:i},o)=>{const l=r.map(p=>`<span class="artist-genre">${p}</span>`).join("");return`
      ...
      `}).join("");V.insertAdjacentHTML("beforeend",e)}function R(){k.classList.remove("is-hidden")}function G(){k.classList.add("is-hidden")}L();A.addEventListener("click",L);const m=document.querySelector(".modal-content"),E=document.querySelector(".modal-close-btn"),c=document.querySelector(".backdrop"),S=document.getElementById("modalLoader");function W(t,e=[]){const{strArtist:s,strArtistThumb:n,intFormedYear:a,intDiedYear:r,strGender:i,intMembers:o,strCountry:l,strBiographyEN:g,genres:p}=t;return`
...
<div class="artist-container">
  ...
  <div class="artist-info">
    ...
    <ul class="info-genres">
      ${(p||[]).map(M=>`<li class="info-genre">${M||""}</li>`).join("")}
    </ul>
   </div>
  </div>
</div>
  ...
  `}function h(t){if(t==null||t==="")return!0;const e=String(t).trim().toLowerCase();return e===""||e==="information missing"}function K(t,e){const s=!h(t),n=!h(e);return!s&&!n?"Information missing":s&&!n?`${t}–present`:!s&&n?`until ${e}`:`${t}–${e}`}async function N(t){c.classList.remove("is-hidden"),document.body.style.overflow="hidden",Y();try{Z();const e=await H(t),a=[...(await P(t)).albumsList||[]].sort((r,i)=>{const o=r.intYearReleased??0;return(i.intYearReleased??0)-o});m.innerHTML=W(e,a)}catch(e){console.log(e),m.innerHTML=`
      <p class="error">Failed to load artist information</p>
    `}finally{j()}}function f(){J(),c.classList.add("is-hidden"),document.body.style.overflow="",m.innerHTML=""}function $(t){t.key==="Escape"&&f()}function B(t){t.target===c&&f()}function Y(){E.addEventListener("click",f),document.addEventListener("keydown",$),c.addEventListener("click",B)}function J(){E.removeEventListener("click",f),document.removeEventListener("keydown",$),c.removeEventListener("click",B)}function Z(){S.classList.remove("is-hidden")}function j(){S.classList.add("is-hidden")}const X=document.getElementById("artistsList");X.addEventListener("click",async t=>{if(t.target.classList.contains("artist-more")){const e=t.target.closest(".artist-card");if(!e)return;const s=e.dataset.id;try{await N(s)}catch(n){console.log(n)}}});function _(t){if(!t)return"--:--";const e=Math.floor(t/1e3),s=Math.floor(e/60),n=e%60;return`${s}:${n.toString().padStart(2,"0")}`}function tt(t){const e=t.movie||t.strMusicVid||t.strVideo||null;if(!e||typeof e!="string")return null;const s=e.trim();return s||null}function et(t){const{strTrack:e,intDuration:s}=t,n=tt(t);return`
    <li class="track">
